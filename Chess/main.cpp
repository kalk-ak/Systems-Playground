...
int main(int argc, char* argv[]) {
	...
	while(!game_over) {
        ...
        // exit the program with return code -1 if an exception is caught here
				...
				try{
				  ...
				  std::cerr << "Could not load board to valid game state" << std::endl;
				  ifs.close();
				  return -1;
				}
				
				// Check that the game is valid
				if (ifs.is_open()){assert(game.is_valid_game());}
	...
	game.clear_board();  // Clear dynamiccally allocated memory from the pieces 
