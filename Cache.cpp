 ...
 */
Cache::Cache(uint32_t sets, uint32_t blocks, uint32_t bytes, bool miss_write_type,
             bool hit_write_type, bool eviction_type)
    : num_sets(sets), num_slots(blocks), num_bytes(bytes), writes_per_block(num_bytes / 4),
      miss_write_type(miss_write_type), hit_write_type(hit_write_type),
      eviction_type(eviction_type), offset_size(log2(bytes)), index_size(log2(num_sets)),
...
Cache::~Cache()
{
    ...
    for (std::vector<Set>::iterator it = cache.begin(); it != cache.end(); ++it)
    {
        Set &current_set = *it;
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                total_cycles += (writes_per_block * memory_cost) + cache_cost + memory_cost;
...
Cache::Slot *Cache::evict(Slot *s, uint32_t new_tag)
{
    if (s->dirty and hit_write_type)
    {
        ...
        total_cycles += (memory_cost * writes_per_block); // Cost to write back to memory
    }
    s->tag = new_tag;
    s->valid = false;
    s->dirty = false;
...
{
    Cache::Slot *new_slot = new Cache::Slot(tag, false, false, current_time, current_time);
    return new_slot;
}
