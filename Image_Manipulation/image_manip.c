...
Image crop(const Image in, Point top_left, Point bot_right)
{
    ...
    // Create a new image data struct because the const image data pointer cannot be pointed to a
    // different image
    Image new_pic = {NULL, 0, 0};
    new_pic.rows = new_rows;
    new_pic.cols = new_cols;
    new_pic.data =
        (Pixel *) malloc(sizeof(Pixel) * new_rows *
                         new_cols); // Allocate a new data pointer for the a number of Pixel structs
    ...
    // Populate the new image data using pointer arthimetic and keeping it within the bounds; bounds
    // will ensure it says within the 2nd point
...
Image zoom_out(const Image in)
{
    ...
    // Check if the image has odd rows or columns, as we will have to disregard them in the new
    // dimensions
    int new_rows;
    int new_cols;
    if ((in.rows % 2) != 0)
    { // Meaning we have an odd value, so disregard the rightmost or bottom col/row
    ...
    zoom_pic.data = (Pixel *) malloc(sizeof(Pixel) * new_cols *
                                     new_rows); // Allocaite Pixel data with new dimensions
    ...
    // Populate the new image data using pointer arthimetic and keeping it within the bounds; will
    // look at 2x2 box at a time
    for (int i = 0; i < new_rows; i++)
    {
        for (int j = 0; j < new_cols; j++)
        {
            ...
            Pixel three =
                in.data[(corner_x + 1) * in.cols + corner_y + 1]; // Shift down 1 and to the right 1

            unsigned char average_r =
                (zero.r + one.r + two.r + three.r) / 4; // Average the rgb values
...
Image pointilism(const Image in, unsigned int seed)
{
    ...
    for (int i = 0; i < in.rows; i++)
    {
        for (int j = 0; j < in.cols; j++)
        {
            ...
            if (point_chance <= 3)
            {                                      // 3 percent chance of getting selected
