...
int main(int argc, char *argv[])
{
    ...
    if (argc < 4)
    {
        fprintf(
            stderr,
            "Image operation not specified. Missing <command-name> argument in command line.\n");
        print_usage();
    ...
    /*
    using an if else statement to identify which task to be performed on image and carries it out if
    valid commands are passed.
    */
    if (strcmp(task, "invert") == 0)
    { // image is to be inverted
    ...
    else if (strcmp(task, "crop") == 0)
    {
        ...
        */
        { // Used to check if there are non-int values passed for the points
            ...
            */
            {
                if (upper_left_col < 0 || upper_left_row < 0 || lower_right_col < 0 ||
                    lower_right_row < 0)
                {
        ...
        if (img_cropped.data == NULL)
        { // Malloc failed in crop
    ...
    else if (strcmp(task, "zoom-out") == 0)
    {
        ...
        if (img_zoomed.data == NULL)
        { // Malloc failed in zoom_out
    ...
    else if (strcmp(task, "binarize") == 0)
    {
        ...
        {
            fprintf(stderr,
                    "Threshold number (%d) out of range for binarize. Must be between 0 and 255 "
                    "inclusive.\n",
                    threshold);
            free_image(&img);
            return RC_OP_ARGS_RANGE_ERR;
        ...
        if (binarized_img.data == NULL)
        { // Malloc failed in binarize
    ...
    else if (strcmp(task, "blur") == 0)
    {
        ...
        if (img_blurred.data == NULL)
        { // Malloc failed in blur
