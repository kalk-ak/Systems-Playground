...
Image read_ppm( FILE *fp ) {
	...
	tag[19] = '\0';
	fscanf( fp , "%19s\n" , tag );
	if( strncmp( tag , "P6" , 20 ) ) { 		// strncmp returns 0 (false) when the two strings are equal
...
int write_ppm( FILE *fp, const Image im ) {  // Input = a file to write to, and an image structure
	...
	fprintf(fp, "P6\n%d %d\n255\n", im.cols, im.rows);  // Format of PPM file include P6 identifier, rgb max vals, and size with columns coming before rows
